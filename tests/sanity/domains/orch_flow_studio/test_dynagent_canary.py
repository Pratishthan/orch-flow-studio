# ABOUTME: Sanity tests for Dynagent canary - validates all Dynagent APIs via Orch Flow Studio.
...
from autobots_orch_flow_studio.domains.orch_flow_studio.call_invoke_agent import (
...
from autobots_orch_flow_studio.domains.orch_flow_studio.get_schema_for_agent import get_schema_for_agent
...
def test_invoke_sync(orch_flow_studio_registered):
...
@pytest.mark.asyncio
async def test_invoke_async(orch_flow_studio_registered):
...
def test_batch(orch_flow_studio_registered):
    """Sanity: batch_invoker via orch_flow_studio_batch."""
    result = orch_flow_studio_batch("joke_agent", ["Tell me a joke"], user_id="test_user")
...
def test_get_schema(orch_flow_studio_registered):
...
def _start_chainlit_no_auth(orch_flow_studio_dir: Path, port: int = 2337) -> subprocess.Popen:
    ...
    env["DYNAGENT_CONFIG_ROOT_DIR"] = str(orch_flow_studio_dir / "agent_configs" / "orch_flow_studio")
    app_path = (
        orch_flow_studio_dir / "src" / "autobots_orch_flow_studio" / "domains" / "orch_flow_studio" / "server.py"
    )
...
def test_ui_chat_script(orch_flow_studio_registered):
    ...
    orch_flow_studio_dir = Path(__file__).resolve().parent.parent.parent.parent.parent
    script_path = Path(__file__).resolve().parent / "fixtures" / "chat_script.yaml"
    with script_path.open() as f:
        script = yaml.safe_load(f)
    messages = script.get("messages", [])

    proc = _start_chainlit_no_auth(orch_flow_studio_dir, port=CHAINLIT_PORT)
    try:
